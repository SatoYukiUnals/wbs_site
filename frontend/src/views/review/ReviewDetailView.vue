...
import { mockReviews, mockReviewComments, mockReviewHistory, mockTasks } from '@/mocks/data'
import type { ReviewComment, ReviewHistory, ReviewStatus } from '@/types'

const route = useRoute()
const authStore = useAuthStore()
const taskId = route.params.taskId as string

const review = mockReviews.find(r => r.task_id === taskId)
...
const comments = ref<ReviewComment[]>(mockReviewComments.filter(c => c.review_id === review?.id))
const history = ref<ReviewHistory[]>(mockReviewHistory.filter(h => h.review_id === review?.id))

const currentStatus = ref<ReviewStatus>(review?.status ?? 'pending')
...
const handleReject = async () => {
  ...
  comments.value.push({
    id: `rc${Date.now()}`,
    review_id: review?.id ?? '',
    body: rejectReason.value,
    author_id: authStore.currentUser?.id ?? '',
    author_name: authStore.currentUser?.display_name ?? '',
    created_at: new Date().toISOString(),
  })
  history.value.push({
    id: `rh${Date.now()}`,
...
<template>
  <div v-if="!review" class="text-gray-500 py-8 text-center">
    レビューが見つかりません
  </div>
  ...
  <div v-else class="max-w-2xl">
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-6 w-80">
        ...
        <div class="flex justify-end gap-2">
          <button class="px-4 py-2 text-sm border rounded hover:bg-gray-50" @click="showApproveDialog = false">
            キャンセル
          </button>
          <button
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-6 w-96">
        ...
        />
        <div class="flex justify-end gap-2">
          <button class="px-4 py-2 text-sm border rounded hover:bg-gray-50" @click="showRejectDialog = false">
            キャンセル
          </button>
          <button
