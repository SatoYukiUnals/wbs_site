...
const allTaskIds = (): string[] => {
  const ids: string[] = []
  const visit = (t: Task) => {
    if (!allChildrenDone(t)) ids.push(t.id)
    for (const c of t.children ?? []) visit(c)
  }
  for (const t of rootTasks.value) visit(t)
  return ids
}
...
/** 削除対象タスクと配下を持つかどうかの判定 */
const deleteTarget = computed(() =>
  findTaskById(rootTasks.value, deleteTargetId.value),
)
const deleteTargetHasChildren = computed(
  () => (deleteTarget.value?.children?.length ?? 0) > 0,
)

const handleDelete = async (mode: 'cascade' | 'promote' = 'cascade') => {
  await api.tasks.delete(projectId, deleteTargetId.value, mode)
...
const BULK_MAX_DEPTH = 4  // 0,1,2,3,4 の5階層
 ...
 * - 直前の行から2段階以上深くなる飛び級は、
 *   間の階層に「(グループ)」という合成項目を自動挿入して保ったまま下げる
 */
const parseBulkText = (text: string): ParsedBulkLine[] => {
  ...
  for (const raw of text.split('\n')) {
    ...
    const target = Math.min(Math.floor(leading / 2), BULK_MAX_DEPTH)
    // 飛び級があれば、間の階層に合成ラッパー項目を挿入する
    while (target > prevDepth + 1) {
      result.push({
        title: '(グループ)',
        depth: prevDepth + 1,
        parentIdx: null,
...
const handleBulkAdd = async () => {
  ...
  for (let d = 0; d <= BULK_MAX_DEPTH; d++) {
    ...
    // 一括作成時は必ず status='Todo' / progress=0 で作成する
      ...
      <div class="ml-auto flex gap-2">
        <template v-if="!isEditMode">
          ...
          <router-link v-if="isAdmin"
            ...
            :to="`/projects/${projectId}/wbs/edit`"
            class="border border-gray-300 text-sky-900 px-3 py-1.5 rounded text-sm transition-colors hover:bg-gray-100">
            編集
          </router-link>
        </template>
            ...
            <tr data-testid="task-row"
              ...
              <td v-if="show('hours')" :class="['sticky z-10 border-r border-b border-gray-500 px-3 py-2 text-xs text-right overflow-hidden', isActiveToday(task) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-white group-hover:bg-gray-50']" :style="{ left: colLeft.hours + 'px' }">
                <input v-if="isEditMode && task.task_type === 'task'" type="number" step="0.5" min="0"
                  ...
                  @change="(e) => { const v = (e.target as HTMLInputElement).value; task.estimated_hours = v === '' ? null : parseFloat(v) }"
              ...
              <td v-if="show('op')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-2 whitespace-nowrap overflow-hidden', isActiveToday(task) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-white group-hover:bg-gray-50']" :style="{ left: colLeft.op + 'px' }">
                ...
                <template v-else>
                  ...
                  <router-link v-if="task.task_kind === 'TMRV' || task.task_kind === 'PJRV'"
            ...
            <template v-if="expandedIds.has(task.id) && task.children">
              <template v-for="child in sortedChildren(task)" :key="child.id">
                <tr data-testid="task-row-child"
                  ...
                  <td v-if="show('hours')" :class="['sticky z-10 border-r border-b border-gray-500 px-3 py-2 text-xs text-right overflow-hidden', isActiveToday(child) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-gray-50 group-hover:bg-gray-100']" :style="{ left: colLeft.hours + 'px' }">
                    <input v-if="isEditMode && child.task_type === 'task'" type="number" step="0.5" min="0"
                      ...
                      @change="(e) => { const v = (e.target as HTMLInputElement).value; child.estimated_hours = v === '' ? null : parseFloat(v) }"
                      class="border border-gray-300 rounded px-1 py-0.5 text-xs w-full text-right" />
                  ...
                  <td v-if="show('op')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-2 whitespace-nowrap overflow-hidden', isActiveToday(child) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-gray-50 group-hover:bg-gray-100']" :style="{ left: colLeft.op + 'px' }">
                    ...
                    <template v-else>
                      ...
                      <router-link v-if="child.task_kind === 'TMRV' || child.task_kind === 'PJRV'"
                ...
                <template v-if="expandedIds.has(child.id) && child.children">
                  <template v-for="grand in sortedChildren(child)" :key="grand.id">
                  <tr data-testid="task-row-grand"
                    ...
                    <td v-if="show('status')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-2 overflow-hidden', isActiveToday(grand) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-50 group-hover:bg-blue-100']" :style="{ left: colLeft.status + 'px' }">
                      <select v-if="grand.task_type === 'task' && !isEditMode" :value="grand.status" :class="statusClass(grand.status)"
                        class="border rounded-full px-2 py-0.5 text-xs cursor-pointer w-full"
                        @change="handleStatusChange(grand, ($event.target as HTMLSelectElement).value as TaskStatus)">
                        <option v-for="s in statusOptions" :key="s" :value="s" class="bg-white text-sky-900">{{ s }}</option>
                      </select>
                      <span v-else-if="grand.task_type === 'task' && isEditMode" :class="statusClass(grand.status)" class="px-2 py-0.5 rounded-full text-xs block text-center">{{ grand.status }}</span>
                    </td>
                    ...
                    <td v-if="show('hours')" :class="['sticky z-10 border-r border-b border-gray-500 px-3 py-2 text-xs text-right overflow-hidden', isActiveToday(grand) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-50 group-hover:bg-blue-100']" :style="{ left: colLeft.hours + 'px' }">
                      <input v-if="isEditMode" type="number" step="0.5" min="0"
                        ...
                        @change="(e) => { const v = (e.target as HTMLInputElement).value; grand.estimated_hours = v === '' ? null : parseFloat(v) }"
                        class="border border-gray-300 rounded px-1 py-0.5 text-xs w-full text-right" />
                    ...
                    <td v-if="show('op')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-2 whitespace-nowrap overflow-hidden', isActiveToday(grand) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-50 group-hover:bg-blue-100']" :style="{ left: colLeft.op + 'px' }">
                      ...
                      <template v-else>
                        ...
                        <router-link v-if="grand.task_kind === 'TMRV' || grand.task_kind === 'PJRV'"
                  ...
                  <!-- ひ孫タスク行（深さ3 = 4階層目） -->
                  <template v-if="expandedIds.has(grand.id) && grand.children">
                    <tr v-for="gg in sortedChildren(grand)" :key="gg.id" data-testid="task-row-gg"
                      :id="`wbs_list__row_${gg.id}`"
                      :class="['border-b border-gray-500 group', isActiveToday(gg) ? 'bg-amber-50 hover:bg-amber-100' : 'bg-blue-100/30 hover:bg-blue-100/50']">
                      <td :class="['sticky left-0 z-10 border-r border-b border-gray-500 px-3 py-2 text-sky-900 text-xs overflow-hidden relative', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']">
                        {{ gg.wbs_no }}<div class="absolute inset-y-0 right-0 w-px bg-gray-100"></div>
                      </td>
                      <td :class="['sticky z-10 border-r border-b border-gray-500 px-3 py-2 text-xs overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.title + 'px' }">
                        <div class="flex items-center gap-1 pl-14">
                          <span class="w-5 flex-shrink-0"></span>
                          <span class="truncate text-sky-900">{{ gg.title }}</span>
                        </div>
                      </td>
                      <td v-if="show('status')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-2 overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.status + 'px' }">
                        <select v-if="gg.task_type === 'task' && !isEditMode" :value="gg.status" :class="statusClass(gg.status)"
                          class="border rounded-full px-2 py-0.5 text-xs cursor-pointer w-full"
                          @change="handleStatusChange(gg, ($event.target as HTMLSelectElement).value as TaskStatus)">
                          <option v-for="s in statusOptions" :key="s" :value="s" class="bg-white text-sky-900">{{ s }}</option>
                        </select>
                        <span v-else-if="gg.task_type === 'task' && isEditMode" :class="statusClass(gg.status)" class="px-2 py-0.5 rounded-full text-xs block text-center">{{ gg.status }}</span>
                      </td>
                      <td v-if="show('assignee')" :class="['sticky border-r border-b border-gray-500 px-3 py-2 text-xs', isEditMode ? 'z-[60] overflow-visible' : 'z-10 overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : memberCellBgClasses(gg) || 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.assignee + 'px' }">
                        <div v-if="isEditMode" class="relative">
                          <button class="text-xs truncate text-left w-full text-sky-900 hover:underline"
                            @click="assigneePopoverId = assigneePopoverId === gg.id ? null : gg.id">
                            {{ draftAssigneeNames(gg) }}
                          </button>
                          <div v-if="assigneePopoverId === gg.id"
                            class="absolute left-0 top-full mt-1 bg-white border border-gray-300 rounded shadow-lg z-[200] py-1 min-w-[140px]">
                            <label v-for="m in members" :key="m.user_id"
                              class="flex items-center gap-2 px-3 py-1.5 text-xs hover:bg-gray-50 cursor-pointer">
                              <input type="checkbox"
                                :checked="gg.assignees.some(a => a.id === m.user_id)"
                                @change="toggleDraftAssignee(gg, m.user_id)" />
                              {{ m.user_name }}
                            </label>
                          </div>
                        </div>
                        <div v-else class="truncate">{{ gg.assignees.map(a => a.name).join(', ') || '—' }}</div>
                      </td>
                      <td v-if="show('start')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-1 overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.start + 'px' }">
                        <template v-if="isEditMode">
                          <input type="date" v-model="gg.start_date" class="border border-gray-300 rounded px-1 py-0.5 text-xs w-full" />
                        </template>
                        <template v-else>
                          <div class="text-xs">{{ formatDate(gg.start_date) }}</div>
                          <div class="text-[10px] text-sky-900 leading-snug">{{ formatDate(gg.actual_start_date) }}</div>
                        </template>
                      </td>
                      <td v-if="show('end')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-1 overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.end + 'px' }">
                        <template v-if="isEditMode">
                          <input type="date" v-model="gg.end_date" class="border border-gray-300 rounded px-1 py-0.5 text-xs w-full" />
                        </template>
                        <template v-else>
                          <div class="text-xs">{{ formatDate(gg.end_date) }}</div>
                          <div class="text-[10px] text-sky-900 leading-snug">{{ formatDate(gg.actual_end_date) }}</div>
                        </template>
                      </td>
                      <td v-if="show('hours')" :class="['sticky z-10 border-r border-b border-gray-500 px-3 py-2 text-xs text-right overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.hours + 'px' }">
                        <input v-if="isEditMode" type="number" step="0.5" min="0"
                          :value="gg.estimated_hours ?? ''"
                          @change="(e) => { const v = (e.target as HTMLInputElement).value; gg.estimated_hours = v === '' ? null : parseFloat(v) }"
                          class="border border-gray-300 rounded px-1 py-0.5 text-xs w-full text-right" />
                        <span v-else>{{ formatHours(gg.estimated_hours) }}</span>
                      </td>
                      <td v-if="show('op')" :class="['sticky z-10 border-r border-b border-gray-500 px-2 py-2 whitespace-nowrap overflow-hidden', isActiveToday(gg) ? 'bg-amber-50 group-hover:bg-amber-100' : 'bg-blue-100 group-hover:bg-blue-200']" :style="{ left: colLeft.op + 'px' }">
                        <template v-if="isEditMode">
                          <button :disabled="!canMoveUp(gg.id)" class="w-5 h-5 rounded hover:bg-gray-100 disabled:opacity-30 text-sky-900 inline-flex items-center justify-center text-xs" @click="moveSibling(gg.id, 'up')">↑</button>
                          <button :disabled="!canMoveDown(gg.id)" class="w-5 h-5 rounded hover:bg-gray-100 disabled:opacity-30 text-sky-900 inline-flex items-center justify-center text-xs ml-1" @click="moveSibling(gg.id, 'down')">↓</button>
                        </template>
                        <template v-else>
                          <span class="w-5 h-5 inline-flex flex-shrink-0"></span>
                          <router-link :to="`/projects/${projectId}/tasks/${gg.id}`"
                            class="rounded hover:bg-blue-100 hover:text-blue-500 transition-colors w-5 h-5 inline-flex items-center justify-center text-sky-900 ml-1"
                            title="編集">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                              <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/>
                          ...
                          <button class="rounded hover:bg-red-100 hover:text-red-500 transition-colors w-5 h-5 inline-flex items-center justify-center text-sky-900 ml-1"
                            title="削除" @click="deleteTargetId = gg.id; showDeleteDialog = true">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                              <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zm0 2h2l.5 1H8.5L9 4zM6 6h8v10H6V6zm2 2a1 1 0 00-1 1v4a1 1 0 102 0V9a1 1 0 00-1-1zm4 0a1 1 0 00-1 1v4a1 1 0 102 0V9a1 1 0 00-1-1z" clip-rule="evenodd"/>
                            </svg>
                          </button>
                          <router-link v-if="gg.task_kind === 'TMRV' || gg.task_kind === 'PJRV'"
                            :to="`/projects/${projectId}/tasks/${gg.id}/reviews`"
                            class="rounded hover:bg-yellow-100 hover:text-yellow-600 transition-colors w-5 h-5 inline-flex items-center justify-center text-sky-900 ml-1"
                            title="レビュー">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
                              <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"/><path fill-rule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm9.707 5.707a1 1 0 00-1.414-1.414L9 12.586l-1.293-1.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                      ...
                      <td v-for="(col, ci) in ganttCols" :key="col.key"
                          :class="['px-0 relative z-0 overflow-hidden h-8 border-r border-b border-gray-500',
                                                                    ganttCellBg(col, ci)]">
                        <div class="absolute inset-x-0 top-1/2 border-t border-gray-500 pointer-events-none"></div>
                        <div v-if="taskBars.get(gg.id) && ci >= taskBars.get(gg.id)!.first && ci <= taskBars.get(gg.id)!.last"
                             :class="['absolute top-1 h-2.5 opacity-65 pointer-events-none z-10', barColor(gg),
                                      ci === taskBars.get(gg.id)!.first ? 'left-px rounded-l-sm' : 'left-0',
                                      ci === taskBars.get(gg.id)!.last  ? 'right-px rounded-r-sm' : 'right-0']">
                        </div>
                        <div v-if="taskActualBars.get(gg.id) && ci >= taskActualBars.get(gg.id)!.first && ci <= taskActualBars.get(gg.id)!.last"
                             :class="['absolute bottom-1 h-2 opacity-60 pointer-events-none', actualBarColor(gg),
                                      ci === taskActualBars.get(gg.id)!.first ? 'left-px rounded-l-sm' : 'left-0',
                                      ci === taskActualBars.get(gg.id)!.last  ? 'right-px rounded-r-sm' : 'right-0']">
                        </div>
                      </td>
                    </tr>
                  </template>
    ...
    <div v-if="showBulkDialog" id="wbs_list__bulk_dialog" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-[520px]">
        ...
        <p class="text-xs text-sky-900 mb-1">
          1行に1タスク。<strong>半角スペース2個 / Tab</strong> でインデントすると下の階層になります（最大5階層）。子を持つ行は自動で親項目として扱われます。
        </p>
    ...
    <div v-if="showDeleteDialog" id="wbs_list__delete_dialog" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-[420px]">
        <p class="text-sky-900 mb-2 font-semibold">
          「{{ deleteTarget?.title ?? 'このタスク' }}」を削除しますか？
        </p>
        <p v-if="deleteTargetHasChildren" class="text-xs text-gray-600 mb-4">
          このタスクには配下があります。削除方法を選択してください。
        </p>
        <div v-if="deleteTargetHasChildren" class="flex flex-col gap-2 mb-4">
          <button
            class="border border-blue-600 text-blue-700 hover:bg-blue-50 rounded px-3 py-2 text-sm text-left"
            @click="handleDelete('promote')"
          >
            <div class="font-semibold">配下を1階層上げる</div>
            <div class="text-xs text-gray-600">この項目だけを削除し、子要素は親項目の直下に移動します。</div>
          </button>
          <button
            class="border border-red-500 text-red-600 hover:bg-red-50 rounded px-3 py-2 text-sm text-left"
            @click="handleDelete('cascade')"
          >
            <div class="font-semibold">配下も含めて全削除</div>
            <div class="text-xs text-gray-600">この項目とその配下のタスクをすべて削除します。</div>
          </button>
        </div>
        <div class="flex justify-end gap-2">
          ...
          <button
            v-if="!deleteTargetHasChildren"
            id="wbs_list__delete_confirm_btn"
            class="px-4 py-2 text-sm text-white bg-red-500 rounded transition-colors hover:bg-red-600"
            @click="handleDelete('cascade')"
          >削除する</button>
        </div>
      </div>
